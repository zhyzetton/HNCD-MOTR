...
class Submitter:
    def __init__(self, dataset_name: str, split_dir: str, seq_name: str, outputs_dir: str, model: nn.Module,
                 ...
                 use_dab: bool = False,
                 visualize: bool = False, det_db = None, save_embeddings_dir = None):
        self.dataset_name = dataset_name
        self.seq_name = seq_name
        # For output file naming, replace / with _ (for AICity22)
        self.seq_name_safe = seq_name.replace("/", "_")
        ...
        self.dataset = SeqDataset(seq_dir=self.seq_dir, det_db=det_db)
    ...
    def run(self):
        ...
        for i, ((image, ori_image, proposals), info) in enumerate(tqdm(self.dataloader, desc=f"Submit seq: {self.seq_name}")):
            ...
            if self.visualize:
                ...
                os.system(f"mv ./outputs/visualize_tmp/hncd/ ./outputs/visualize_tmp/frame_{i+1}/")
    ...
    def write_results(self, tracks_result: TrackInstances, frame_idx: int):
        with open(os.path.join(self.predict_dir, f"{self.seq_name_safe}.txt"), "a") as file:
            for i in range(len(tracks_result)):
...
def submit(config: dict):
    ...
    # NOTE track embedding
    save_embddings_dir = config["SAVE_EMBEDDINGS_DIR"]
    ...
    elif dataset_name == "AICity22":
        # AICity22 has two-level structure: scenario/camera
        aicity_root = path.join(data_root, "AICity22_Track1_MTMC_Tracking")
        if dataset_split == "val":
            data_split_dir = path.join(aicity_root, "validation")
        else:
            data_split_dir = path.join(aicity_root, dataset_split)
    else:
        data_split_dir = path.join(data_root, dataset_name, "images", dataset_split)

    # Get sequence names
    if dataset_name == "AICity22":
        # For AICity22, seq_names are scenario/camera combinations
        # Skip symlinks to avoid duplicate processing
        seq_names = []
        for scenario in os.listdir(data_split_dir):
            scenario_path = path.join(data_split_dir, scenario)
            # Skip symlinks (e.g., S02_c006 -> S02/c006)
            if path.isdir(scenario_path) and not path.islink(scenario_path):
                for camera in os.listdir(scenario_path):
                    camera_path = path.join(scenario_path, camera)
                    if path.isdir(camera_path) and not path.islink(camera_path):
                        seq_names.append(f"{scenario}/{camera}")
    else:
    ...
    for seq_name in seq_names:
        ...
        submitter = Submitter(
            ...
            miss_tolerance=miss_tolerance,
            det_db=det_db,
            save_embeddings_dir=save_embddings_dir
        )
        submitter.run()
    return
