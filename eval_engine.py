...
def eval_model(model: str, eval_dir: str, data_root: str, dataset_name: str, data_split: str, threads: int, port: int,
    ...
    if dataset_name == "DanceTrack" or dataset_name == "SportsMOT":
        ...
        aicity_root = os.path.join(data_root, "AICity22_Track1_MTMC_Tracking")
        if data_split == "val":
            gt_dir = os.path.join(aicity_root, "validation")
        else:
            gt_dir = os.path.join(aicity_root, data_split)
    else:
    ...
    if dataset_name == "DanceTrack" or dataset_name == "SportsMOT":
        ...
        # For AICity22, use the provided evaluation script or TrackEval with custom seqmap
        aicity_root = os.path.join(data_root, "AICity22_Track1_MTMC_Tracking")
        seqmap_file = os.path.join(aicity_root, f'{data_split}_seqmap.txt')
        # Disable preprocessing for AICity22 since it uses class=-1 format
        os.system(f"python3 TrackEval/scripts/run_mot_challenge.py --SPLIT_TO_EVAL {data_split}  "
                  f"--METRICS HOTA CLEAR Identity  --GT_FOLDER {gt_dir} "
                  f"--SEQMAP_FILE {seqmap_file} "
                  f"--SKIP_SPLIT_FOL True --TRACKERS_TO_EVAL '' --TRACKER_SUB_FOLDER ''  --USE_PARALLEL True "
                  f"--NUM_PARALLEL_CORES 8 --PLOT_CURVES False --DO_PREPROC False "
                  f"--TRACKERS_FOLDER {tracker_mv_dir}")
    else:
