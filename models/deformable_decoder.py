...
class DeformableDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos, query_mask, src_padding_mask, num_proposals, num_cdn, attn_mask=None):
        """
        Args:
...
class DeformableDecoderLayer(nn.Module):
    ...
    def forward_self_attn(self, tgt, query_pos, query_mask, attn_mask):
        q = k = self.with_pos_embed(tgt, query_pos)
        tgt2, _ = self.self_attn(q, k, tgt, key_padding_mask=query_mask, attn_mask=attn_mask)
        if self.visualize:
            torch.save(_[0, :, :].cpu(),
    ...
    def forward_track_attn(self, tgt, query_pos, query_mask, num_proposals):
        q = k = self.with_pos_embed(tgt, query_pos)
        num_det_query = self.n_det_queries + num_proposals
        not_track = num_det_query
        if q.shape[1] > not_track:
            tgt2, _ = self.track_attn(q[:, not_track:], k[:, not_track:], tgt[:, not_track:],
                                      key_padding_mask=query_mask[:, not_track:])
            tgt2 = self.norm4(tgt[:, not_track:] + self.dropout5(tgt2))
            tgt = torch.cat([tgt[:, :not_track], tgt2], dim=1)
        return tgt
    ...
    def forward(self, tgt, query_pos, reference_points,
                ...
                merge_det_track=False, num_proposals=0, num_cdn=0, attn_mask=None):
        """
        Args:
            tgt:                    (B, Nq, C)
            query_pos:              (B, Nq, C)
        ...
        if merge_det_track is False:
            not_track = num_cdn + self.n_det_queries + num_proposals
            track_tgt = tgt[:, not_track:, :]      # (B, Nq_track, C)
            tgt = tgt[:, :not_track, :]            # (B, Nq_det, C)
            query_pos = query_pos[:, :not_track, :]
            reference_points = reference_points[:, :not_track, :, :]
            query_mask = query_mask[:, :not_track]
            attn_mask = attn_mask[:not_track, :not_track] if attn_mask is not None else None

        # 默认False
        if self.extra_track_attn:
            tgt = self.forward_track_attn(tgt=tgt, query_pos=query_pos, query_mask=query_mask, num_proposals=num_proposals)
        tgt = self.forward_self_attn(tgt=tgt, query_pos=query_pos, query_mask=query_mask, attn_mask=attn_mask)
        # Cross Attention
