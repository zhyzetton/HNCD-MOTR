...
class ClipCriterion:
    def __init__(self, num_classes, matcher: HungarianMatcher, n_det_queries, aux_loss: bool, weight: dict,
                 ...
                 num_cdn_group: int = 3, cdn_weights: List = None):
        """
    ...
    def init_a_clip(self, batch: Dict, hidden_dim: int, num_classes: int, device: torch.device):
        ...
        if self.num_cdn_group > 0:
            self.loss['cdn_box_l1_loss'] = torch.zeros(()).to(self.device)
            self.loss['cdn_box_giou_loss'] = torch.zeros(()).to(self.device)
            self.loss['cdn_label_focal_loss'] = torch.zeros(()).to(self.device)
        return
    ...
    def get_mean_by_n_gts(self) -> Tuple[Dict, Dict]:
        ...
        loss = {}
        for k in self.loss:
            if 'cdn' in k and self.cdn_meta is not None:
                loss[k] = self.loss[k] / (total_n_gts * self.cdn_meta['cdn_num_group'])
            else:
        ...
        for k in self.log:
            for i in range(len(n_gts)):
                ...
                if f"cdn{i}" in k and self.cdn_meta is not None:
                    log[k] = (self.log[k] / (n_gts[i] * self.cdn_meta['cdn_num_group']), 1)
                    break
        return loss, log
    
    # NOTE 获取对比去噪正例indices
    def get_cdn_matched_indices(self, dn_meta, targets):
        """get_cdn_matched_indices"""
        dn_positive_idx, dn_num_group = dn_meta["cdn_positive_idx"], dn_meta["cdn_num_group"]
        
        num_gts = [len(t["labels"]) for t in targets]
        device = targets[0]["labels"].device
        dn_positive_idx[0] = dn_positive_idx[0].to(device)
    ...
    def process_single_frame(self, model_outputs: dict, tracked_instances: List[TrackInstances], frame_idx: int, num_proposals: int):
        """
        ...
        """
        if 'cdn_meta' in model_outputs:
            self.cdn_meta = model_outputs['cdn_meta']
        ...
        tracked_instances = self.update_tracked_instances(model_outputs=model_outputs,
                                                          tracked_instances=tracked_instances, num_proposals=num_proposals)

        # 3. Get the detection results in current frame.
        detection_res = {
            "pred_logits": model_outputs["pred_logits"][:, :self.n_det_queries + num_proposals, :].detach(),    # (B, Nd, n_classes)
            "pred_boxes": model_outputs["pred_bboxes"][:, :self.n_det_queries + num_proposals, :].detach()      # (B, Nd, 4)
        }
        ...
        for b in range(len(tracked_instances)):
            tracked_outputs_idx = torch.arange(start=self.n_det_queries + num_proposals,
                                               end=self.n_det_queries + num_proposals + len(tracked_instances[b]))
        ...
        if self.aux_loss:
            for i, aux_outputs in enumerate(model_outputs["aux_outputs"]):
                # Same to 3.
                aux_det_res = {
                    "pred_logits": aux_outputs["pred_logits"][:, :self.n_det_queries + num_proposals, :].detach(),
                    "pred_boxes": aux_outputs["pred_bboxes"][:, :self.n_det_queries + num_proposals, :].detach()
                }
                # Same to 5.
        ...
        # Compute cdn loss.
        if 'cdn_outputs' in model_outputs:
            current_gt = gt_trackinstances[0]
            target = {
                'boxes': current_gt.boxes,
                'labels': current_gt.labels
            }
            
            indices_cdn = self.get_cdn_matched_indices(model_outputs["cdn_meta"], [target])
            for i, cdn_outputs in enumerate(model_outputs['cdn_outputs']):
                cdn_loss_label = self.get_loss_label(outputs=cdn_outputs,
                                                     gt_trackinstances=gt_trackinstances,
                                                     idx_to_gts_idx=indices_cdn)
                cdn_loss_l1, cdn_loss_giou = self.get_loss_box(outputs=cdn_outputs,
                                                                gt_trackinstances=gt_trackinstances,
                                                                idx_to_gts_idx=indices_cdn)
                self.loss["cdn_box_l1_loss"] += cdn_loss_l1 * self.frame_weights[frame_idx] * self.cdn_weights[i]
                self.loss["cdn_box_giou_loss"] += cdn_loss_giou * self.frame_weights[frame_idx] * self.cdn_weights[i]
                self.loss["cdn_label_focal_loss"] += cdn_loss_label * self.frame_weights[frame_idx] * self.cdn_weights[i]
        if self.num_cdn_group > 0:
            self.log[f"cdn{frame_idx}_box_l1_loss"] = self.loss["cdn_box_l1_loss"].item()
            self.log[f"cdn{frame_idx}_box_giou_loss"] = self.loss["cdn_box_giou_loss"].item()
            self.log[f"cdn{frame_idx}_label_focal_loss"] = self.loss["cdn_label_focal_loss"].item()
    ...
    def update_tracked_instances(self, model_outputs: dict, tracked_instances: List[TrackInstances], num_proposals)\
            -> List[TrackInstances]:
        """
        ...
        for b in range(len(tracked_instances)):
            if len(tracked_instances[b]) > 0:
                track_mask = model_outputs["query_mask"][b][self.n_det_queries + num_proposals:]
                tracked_instances[b].boxes = model_outputs["pred_bboxes"][b][self.n_det_queries + num_proposals:][~track_mask]
                tracked_instances[b].logits = model_outputs["pred_logits"][b][self.n_det_queries + num_proposals:][~track_mask]
                # Query embed and ref_pts will be updated in the query_updater module.
                tracked_instances[b].output_embed = model_outputs["outputs"][b][self.n_det_queries + num_proposals:][~track_mask]
...
def build(config: dict):
    ...
    return ClipCriterion(
        ...
        cdn_weights=config["CDN_LOSS_WEIGHT"],
    )
