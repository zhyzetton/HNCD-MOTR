...
def update_config(config: dict, option: argparse.Namespace) -> dict:
    ...
    # Parameters to skip (automatically set by torchrun or not needed in config)
    skip_params = {"config_path", "local_rank", "local_rank"}

    for option_k, option_v in vars(option).items():
        if option_k not in skip_params and option_v is not None:
