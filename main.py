...
def parse_option():
    ...
    parser.add_argument("--local-rank", type=int, default=0, help="Local rank for distributed training (automatically set by torchrun)")
...
def main(config: dict):
    ...
    from train_accident_classifier import train as train_classifier
    from train_accident_sotad import train as train_sotad
    if config["MODE"] == "train":
        train(config=config)
    elif config["MODE"] == "train_classifier":
        train_classifier(config=config)
    elif config["MODE"] == "train_sotad":
        train_sotad(config=config)
