...
def train_one_epoch(model: HNCD, train_states: dict, max_norm: float,
    ...
    for i, batch in enumerate(dataloader):
        '''
            batch: { imgs:[[...]], infos: [ [ { boxes, ids, labels, areas, frame_idx, scores, unnorm_img }, ... ] ], proposals: [[...]] }
        '''
        ...
        for frame_idx in range(len(batch["imgs"][0])):
            if no_grad_frames is None or frame_idx >= no_grad_frames:
                ...
                proposals = batch['proposals'][0][frame_idx].to(device)
                gt_target = batch['infos'][0][frame_idx]
                res = model(frame=frame, tracks=tracks, proposals=proposals.unsqueeze(0), gt_target=gt_target)
